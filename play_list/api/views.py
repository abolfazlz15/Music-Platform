...
class UserPlayListView(APIView):
    ...
    def get(self, request, pk):
        queryset = (
            ...
            .filter(user__id=pk)
            .annotate(number_of_songs=Count("songs"))
            .order_by("-id")
        )
...
class UserCreatePlayListView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            serializer.save()
            return Response(
                {"result": "playlist created", "success": True},
                status=status.HTTP_201_CREATED,
            )
        return Response(
            {"result": serializer.errors, "success": False},
...
class PlaylistAddMusicView(generics.UpdateAPIView):
    ...
    permission_classes = [
        ...
        custom_permissions.AddMusicToPlaylistSubscriber,
    ]
    ...
    def put(self, request, pk, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"message": "Invalid music ID", "success": False},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {"result": "music added", "data": serializer.data, "success": True},
...
class PlaylistRemoveMusicView(generics.DestroyAPIView):
    ...
    def delete(self, request, pk):
        ...
        serializer = self.get_serializer(playlist)
        return Response(
            {"result": "music remove", "data": serializer.data, "success": True},
            status=status.HTTP_204_NO_CONTENT,
        )
...
class ApprovedPlaylistView(APIView):
    ...
    def get(self, request):
        international_playlist = ApprovedPlaylist.objects.filter(
            is_international=True
        ).order_by("-id")[:5]
        iranian_playlist = ApprovedPlaylist.objects.filter(
            is_international=False
        ).order_by("-id")[:5]
        iranian_playlist_serializer = serializers.ApprovedPlaylistSerializer(
            iranian_playlist, many=True, context={"request": request}
        )
        international_playlist_serializer = serializers.ApprovedPlaylistSerializer(
            international_playlist, many=True, context={"request": request}
        )
